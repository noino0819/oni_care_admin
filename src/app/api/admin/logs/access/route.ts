...
import type { AccessLog, ApiResponse, PaginationInfo } from '@/types';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const dataQuery = `
      SELECT 
        id, user_id, user_name, device_type, os, browser,
    ...
    const data = await query<AccessLog>(dataQuery, [...params, limit, offset]);
    ...
    return NextResponse.json<ApiResponse<AccessLog[]>>({
      success: true,
      data,
      pagination,
    });
  } catch (error) {
    console.error('접속로그 조회 에러:', error);
    return NextResponse.json<ApiResponse<null>>({
      success: false,
      error: { code: 'FETCH_ERROR', message: '접속로그 조회 중 오류가 발생했습니다.' },
    }, { status: 500 });
  }
}
