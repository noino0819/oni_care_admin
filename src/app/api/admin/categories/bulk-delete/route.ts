...
import { withAppTransaction } from '@/lib/app-db';
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await withAppTransaction(async (client) => {
      // 연결된 중분류 삭제
      await client.query(
        `DELETE FROM content_subcategories WHERE category_id = ANY($1::int[])`,
        [ids]
      );
      // 대분류 삭제
      await client.query(
        `DELETE FROM content_categories WHERE id = ANY($1::int[])`,
        [ids]
      );
    });
